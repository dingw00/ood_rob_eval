...
TEST_TRANSFORMS_AWA2 = lambda size: tf.Compose([
        tf.Resize((size, size)),
        tf.ToTensor(),
    ])
TRAIN_TRANSFORMS_AWA2 = lambda size: tf.Compose([
                                    tf.RandomRotation(40),                # Rotation by 40 degrees
                                    tf.RandomHorizontalFlip(),           # Horizontal flip with 50% probability
                                    tf.RandomAffine(degrees=0,           # Random affine transformation: shear, scale, and translate
                                                            translate=(0.2, 0.2), 
                                                            shear=0.2,
                                                            scale=(0.8, 1.2)),
                                    tf.ColorJitter(brightness=0.2,       # Random color jitter: brightness, contrast, saturation, and hue
                                                        contrast=0.2, 
                                                        saturation=0.2, 
                                                        hue=0.2),
...
class SubsetImageFolder(tv.datasets.ImageFolder):
    def __init__(self, root, transform=None, target_transform=None, subset_classes=None):
        super(SubsetImageFolder, self).__init__(root, transform=transform, target_transform=target_transform)

        if subset_classes is not None:
            class_mapping = dict()
            for k, v in self.class_to_idx.items():
                if v in subset_classes:
                    class_mapping[v] = len(class_mapping)

            self.class_to_idx = {k: class_mapping[v] for k, v in self.class_to_idx.items() if v in subset_classes}
            self.targets = [class_mapping[target] for target in self.targets if target in subset_classes]
            self.samples = [(path, class_mapping[class_idx]) for path, class_idx in self.samples if class_idx in subset_classes]

            print("Image net classes:", len(subset_classes))
            print("Image net class_to_idx:", len(self.class_to_idx), self.class_to_idx)
...
def load_dataset(data_dir, dataset_name, benchmark, batch_size=1, split=None, 
    ...
    if normalize:
        tfs = tf.Compose([tfs, tf.Normalize(mean, std)])
    
    dataset = load_dataset_from_torchvision(data_dir, dataset_name, transform=tfs, 
...
def load_dataset_from_folder(data_dir, dataset_name, transform=None, split=None):
    ...
    if dataset_name == "Imagenet100":
        subset_classes = list(range(0, 1000, 10))  # 0, 10, 20, ..., 990
    
        dataset = SubsetImageFolder(data_dir+"imagenet-1k/"+split, 
                                    subset_classes=subset_classes,
                                    transform=transform)
    else:
        dataset_name = dataset_name.replace("Imagenet1k", "imagenet-1k")
        dataset = tv.datasets.ImageFolder(data_dir+dataset_name+"/"+split, 
                                          transform=transform)
    
    return dataset

def load_dataset_from_torchvision(save_dir, dataset_name, transform=None, split=None, download=True):
    ...
    if dataset_name == "CIFAR10":
        ...
        if os.path.exists(os.path.join(save_dir, dataset_name)):
            download = False
    ...
    #     dataset = tv.datasets.FakeData(size=10000,
